#!/bin/bash
# Bengali CosyVoice3 staged runner.
#
# Run this file from examples/bengali/cosyvoice3.
# Example:
#   bash run.sh --stage 0 --stop_stage 5 --dataset_root /home/kawshik/TTS_Dataset
...
# Export once before any stage starts so embedding, token extraction,
# parquet preparation, and training inherit the same GPU selection.
export CUDA_VISIBLE_DEVICES="${cuda_visible_devices}"
echo "Using CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES}"

if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
  echo "Stage 0: Prepare Bengali metadata"
  prep_args=(
  ...
  python local/prepare_bengali_data.py "${prep_args[@]}"
fi
